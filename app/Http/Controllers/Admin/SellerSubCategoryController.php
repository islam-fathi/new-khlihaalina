...
class sellerSubCategoryController extends Controller
{
    public function index( Request $request )
    {
        ...
        if($request->has('search'))
        {
            ...
            $categories = SellerCategories::where(['position'=>1])->where(function ($q) use ($key) {
            ...
            $categories=SellerCategories::where(['position'=>1]);
        }
        $categories = $categories->latest()->paginate(Helpers::pagination_limit())->appends($query_param);
        return view('admin-views.seller.category.sub-category-view',compact('categories','search'));
    }
    ...
    public function store(Request $request)
    {
        ...
        Toastr::success('Seller Sub Category updated successfully!');
        return back();
    }
    ...
    {
        if ($request->ajax()) {
            $data = SellerCategories::where('position', 1)->orderBy('id', 'desc')->get();
            return response()->json($data);
        }
    }
}
