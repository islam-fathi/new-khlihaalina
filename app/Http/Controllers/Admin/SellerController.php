...
class SellerController extends Controller
{
    public function index(Request $request)  
    {
        ...
        if ($request->has('search')) {
            ...
            $sellers = Seller::with(['orders', 'product','cities'])
    ...
    public function view(Request $request, $id, $tab = null)
    {
        ...
        if ($tab == 'order') {
            ...
            $seller_shop = Shop::where("seller_id" , $id)->get();
            $seller_categories = json_decode($seller_shop[0]->category_ids);
            // return  $seller_categories;
            $categories = SellerCategories::where(['parent_id' => 0])->get();
            ...
            if ($request->has('category_id')) {
                ...
                $seller_shop = Shop::where("seller_id" , $id)->first();
                $seller_shop->category_ids = json_encode($category);
                $seller_shop->save(); 
                Toastr::success('Category for this seller has been updated.');
            }

            //return back(); 
            return view('admin-views.seller.view.setting', compact('seller','seller_categories','categories'));
    ...
    public function add_seller()
    {
        ...
        return view('admin-views.seller.add-new-seller',compact('cities','cat'));
    }

    public function get_categories(Request $request)
    {
        $cat = SellerCategories::where(['parent_id' => $request->parent_id])->get();
