...
class UserProfileController extends Controller
{
    ...
    {
        $doctor_appointments = DoctorsAppointments::where('request_by_id', auth('customer')->id())->with(['doctors'])->orderBy('id','DESC')->get();
        return view('web-views.users-profile.doctor-appointments', compact('doctor_appointments'));
    }

    public function pharmacy_orders()
    {
        $pharmacy_orders = PharmaciesOrders::where('request_by_id', auth('customer')->id())->with(['pharmacy'])->orderBy('id','DESC')->get();
        return view('web-views.users-profile.pharmacy-orders', compact('pharmacy_orders'));
    }

    public function service_orders() 
    {
        $service_orders = ServicesOrders::where('request_by_id', auth('customer')->id())->with(['service'])->orderBy('id','DESC')->get();
        return view('web-views.users-profile.service-orders', compact('service_orders'));
    }
    ...
    public function doctor_appointment_details(Request $request) 
    {
        $doctor_appointment = DoctorsAppointments::with(['doctors'])->find($request->id);
        return view('web-views.users-profile.doctor-appointment-details', compact('doctor_appointment'));
    }

    public function pharmacy_order_details(Request $request)
    {
        $pharmacy_order = PharmaciesOrders::with(['pharmacy'])->find($request->id);
        return view('web-views.users-profile.pharmacy-order-details', compact('pharmacy_order'));
    }

    public function service_order_details(Request $request)
    {
        $service_order = ServicesOrders::with(['service'])->find($request->id);
        return view('web-views.users-profile.service-order-details', compact('service_order'));
    }
    ...
    public function submit_review_type(Request $request,$type ,$id)
    {
        if($type == 'doctor'){
            $data_details = DoctorsAppointments::find($id);
        }elseif($type == 'pharmacy'){
            $data_details = PharmaciesOrders::find($id);
        }elseif($type == 'service'){
            $data_details = ServicesOrders::find($id);
        }
        
        return view('web-views.users-profile.submit-review-other-type',compact('type','data_details')); 
