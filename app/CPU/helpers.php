...
class Helpers
{
    ...
    {
        if (strpos(url()->current(), '/api')) {
            // $city = App::getDefaultCity();
            $data = Helpers::get_business_settings('default_city');  
            session()->put('default_city_settings', $data);
            $city = $data;
        } elseif (session()->has('default_city_settings')) {
            $city = session('default_city_settings');
        } else {
            $data = Helpers::get_business_settings('default_city');  
            session()->put('default_city_settings', $data);
            $city = $data;
    ...
    {
        $config = null;
        $check = ['currency_model', 'currency_symbol_position', 'system_default_currency', 'language', 'company_name','decimal_point_settings','default_city'];
    ...
    public static function decode_images_formatting($data, $multi_data = false) 
    {
        $storage = [];
        if ($multi_data == true) {
            foreach ($data as $item) {
    ...
    public static function seller_data_formatting($data, $multi_data = false)
    {
        $storage = [];
        if ($multi_data == true) {
            foreach ($data as $item) {
    ...
    public static function default_city_load() 
    {
        if (\session()->has('default_city_settings')) {
            $default_city = \session('default_city_settings');
        } else {
            $default_city = BusinessSetting::where('type', 'default_city')->first();
            \session()->put('default_city_settings', $default_city);
        }
        return $default_city;
    }
