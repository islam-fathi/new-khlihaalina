            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        {{ \App\CPU\translate('seller category form') }}
                    </div>
                    <div class="card-body"
                        style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};">
                        <form action="{{ route('admin.seller-category.update', [$category['id']]) }}" method="POST"
                                ...
                                <div class="col-12 col-md-3">
                                    <div class="form-group">
                                        <label class="input-label"
                                            for="priority">{{ \App\CPU\translate('choose_priority_number') }}</label>
                                        <select class="form-control" name="priority" id="" required>
                                            @for ($i = 0; $i <= 10; $i++)
                                                <option value="{{ $i }}"
                                                    {{ $category['priority'] == $i ? 'selected' : '' }}>{{ $i }}
                                                </option>
                                            @endfor
                                        </select>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <center>
                                                <img style="width: 20%;border: 1px solid; border-radius: 10px;"
                                                    id="viewer"
                                                    src="{{ asset('storage/app/public/seller/category') }}/{{ $category['icon'] }}"
                                                    alt="" />
                                            </center>
                                        </div>
                                    </div>
