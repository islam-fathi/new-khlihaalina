...
@section('title', \App\CPU\translate('Seller Category'))
    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{ \App\CPU\translate('Seller category') }}</li>
            </ol>
        </nav>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        {{ \App\CPU\translate('seller_category_form') }}
                    </div>
                    <div class="card-body"
                        style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};">
                        <form action="{{ route('admin.seller-category.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                                    ...
                                    @foreach (json_decode($language) as $lang)
                                        <div class="form-group {{ $lang != $default_lang ? 'd-none' : '' }} lang_form"
                                            ...
                                            <input type="text" name="name[]" class="form-control"
                                                placeholder="{{ \App\CPU\translate('New') }} {{ \App\CPU\translate('Seller Category') }}"
                    ...
                    <div class="card-header">
                        <div class="row flex-between justify-content-between align-items-center flex-grow-1">
                            <div class="col-12 col-sm-6 col-md-6">
                                <h5>{{ \App\CPU\translate('seller category table') }} <span
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <img class="rounded" width="64"
                                                    ...
                                                    src="{{ asset('storage/app/public/seller/category') }}/{{ $category['image'] }}">
                                            </td>
                                            <td>
                                                {{ $category['priority'] }}
                                            </td>
...
@push('script')
    ...
    <script>
        $(document).on('change', '.category-status', function() {
            ...
            $.ajax({
                url: "{{ route('admin.seller-category.status') }}",
                method: 'POST',
                data: {
                    id: id,
                    home_status: status
        ...
        $(document).on('click', '.delete', function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{ route('admin.seller-category.delete') }}",
                        method: 'POST',
                        data: {
                            id: id
                        },
                        success: function() {
                            toastr.success(
                                '{{ \App\CPU\translate('Seller_Category_deleted_Successfully.') }}'
                                );
                            location.reload();
                        }
                    });
