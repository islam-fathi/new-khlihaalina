...
@section('title', \App\CPU\translate('Specialties'))
    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{ \App\CPU\translate('Specialties') }}</li>
            </ol>
        </nav>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        {{ \App\CPU\translate('speciality_form') }}
                    </div>
                    <div class="card-body"
                        style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};">
                        <form action="{{ route('admin.doctor.add-speciality') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                                    ...
                                    @foreach (json_decode($language) as $lang)
                                        <div class="form-group {{ $lang != $default_lang ? 'd-none' : '' }} lang_form"
                                            ...
                                            <label class="input-label"
                                                for="exampleFormControlInput1">{{ \App\CPU\translate('speciality') }}
                                                ({{ strtoupper($lang) }})
                                            </label>
                                            <input type="text" name="speciality[]" class="form-control"
                                                placeholder="{{ \App\CPU\translate('New') }} {{ \App\CPU\translate('speciality') }}"
                        ...
                        <div class="table-responsive">
                            <table style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};"
                                ...
                                <thead class="thead-light">
                                    <tr>
                                        <th style="width: 100px">{{ \App\CPU\translate('speciality') }}
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <label class="switch switch-status">
                                                    <input type="checkbox" class="speciality-status"
                                                        id="{{ $speciality['id'] }}"
                                                        {{ $speciality->status == 1 ? 'checked' : '' }}>
                                            ...
                                            <td>
                                                <a class="btn btn-primary btn-sm edit" style="cursor: pointer;"
                                                    ...
                                                    href="{{ route('admin.doctor.edit-speciality', [$speciality['id']]) }}">
                                                    <i class="tio-edit"></i>
                                                </a>
...
@push('script')
    ...
    <script>
        $(document).on('change', '.speciality-status', function() {
            var id = $(this).attr("id");
            ...
            $.ajax({
                url: "{{ route('admin.doctor.status-speciality') }}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
        ...
        $(document).on('click', '.delete', function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{ route('admin.doctor.delete-speciality') }}",
                        method: 'POST',
                        data: {
                            id: id
                        },
