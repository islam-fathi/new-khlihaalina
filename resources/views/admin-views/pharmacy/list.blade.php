            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <div>
                            <h5>{{ \App\CPU\translate('Pharmacies table') }}
                        ...
                        <div>
                            <a href="{{ route('admin.pharmacy.add') }}" class="btn btn-primary  float-right">
                            ...
                            <table id="datatable"
                                ...
                                <tbody>
                                    ...
                                    <tr class="">
                                        ...
                                        <td>
                                            @isset($pharmacy->cities['name'])
                                                {{$pharmacy->cities['name']}}  
                                            @endisset
                                        </td>
                                        <td>{{$pharmacy->address}}</td>
                                        <td>{{$pharmacy->working_from_time}}</td>
                                        <td>{{$pharmacy->working_to_time}}</td>
                                        ...
                                        <td>
                                            <a class="btn btn-primary btn-sm edit" style="cursor: pointer;"
                                                ...
                                                href="{{ route('admin.pharmacy.edit', [$pharmacy['id']]) }}">
                                                <i class="tio-edit"></i>
                                            </a>
...
@push('script')
    ...
    <script>
        $(document).on('change', '.status', function() {
            ...
            $.ajax({
                url: "{{ route('admin.pharmacy.status') }}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
                ...
                success: function(data) {
                    if (data.success == true) {
                        toastr.success('{{ \App\CPU\translate('Pharmacy updated successfully') }}');
                    }
                }
            });
        });
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function() {
                            toastr.success(
                                '{{ \App\CPU\translate('Pharmacy Deleted Successfully.') }}'
                                );
                            location.reload();
                        }
                    });
