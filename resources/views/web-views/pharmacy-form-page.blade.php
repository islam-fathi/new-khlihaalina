...
@section('title', \App\CPU\translate('Pharmacy Page'))

@push('css_or_js') 
    @if ($pharmacy['id'] != 0)
        <meta property="og:image" content="{{ asset('storage/app/public/pharmacy') }}/{{ $pharmacy->image }}" />
        <meta property="og:title" content="{{ $pharmacy->name }} " />
        <meta property="og:url" content="{{ route('pharmacyView', [$pharmacy['id']]) }}">
    @else
    ...
    @if ($pharmacy['id'] != 0)
        <meta property="twitter:card" content="{{ asset('storage/app/public/pharmacy') }}/{{ $pharmacy->image }}" />
        <meta property="twitter:title" content="{{ route('pharmacyView', [$pharmacy['id']]) }}" />
        <meta property="twitter:url" content="{{ route('pharmacyView', [$pharmacy['id']]) }}">
    @else
    ...
    {{-- <link rel="stylesheet" href="../public/assets/front-end/css/home.css" />
    <link rel="stylesheet" href="../public/assets/front-end/css/responsive1.css" /> --}}
    <meta name="_token" content="GsVtXnctAybsItALcPf3XFNqChBQNNhqI5zWbb1T">
@endpush
@section('content')
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body" style="padding: 3px!important; cursor: pointer"
                        onclick="location.href='{{ route('pharmacyView',[$pharmacy->id]) }}'">
        ...
        $rating = \App\CPU\ProductManager::get_rating($pharmacy->reviews);
        ?>
                ...
                <div class="col-md-9 col-12">
                    <div class="row">
                                        ...
                                        @foreach (json_decode($pharmacy->images) as $key => $photo)
                                            <div
                                                ...
                                                <img class="cz-image-zoom img-responsive" style="width:100%;height:auto"
                                                    ...
                                                    data-zoom="{{asset("storage/app/public/pharmacies/$photo")}}"
                                                    alt="Pharmacy image" width="">
                        ...
                        <div class="col-lg-7 col-md-8 col-12 mt-md-0 mt-sm-3" style="direction: ltr">
                            <div class="details">
                                @if(!empty($pharmacy->working_from_time)  && !empty($pharmacy->working_to_time))
                                    @php($destinationTimezone = new DateTimeZone('Africa/Cairo'))
                                    @php($now_time = now()->setTimeZone($destinationTimezone)->format('H:i'))
                                    {{-- @php($now_time = "15:00") --}}
                                    @if($now_time >= $pharmacy->working_from_time)
                                        @if($pharmacy->working_to_time <= $pharmacy->working_from_time)
                                            @php($is_open = true)
                                        @else
                                            @if($now_time < $pharmacy->working_to_time)
                                                @php($is_open = true)
                                            @else
                                                @php($is_open = false)
                                            @endif
                                ...
                                <span class="mb-2" style="font-size: 22px;font-weight:700;">{{ $pharmacy->name }}</span>
                                        ...
                                        <ul class="nav nav-tabs d-flex justify-content-center" role="tablist"
                                            ...
                                            <li class="nav-item">
                                                <a class="nav-link active " href="#overview" data-toggle="tab"
                                                    ...
                                                    {{ \App\CPU\translate('About Pharmacy') }}
                                                </a>
                                            </li>
                                            <li class="nav-item">
                                                ...
                                                <div class="tab-pane fade show active" id="overview" role="tabpanel">
                                                    <div class="row pt-2 specification">
                                                        <div class="text-body col-lg-12 col-md-12"
                                                            ...
                                                            <h2>{{ \App\CPU\translate('Pharmacy Details') }}</h2>
                                                            <ul>
                                                                <li>{{ \App\CPU\translate('Name') }} : {{ $pharmacy->name }}</li>
                                                                <li>{{ \App\CPU\translate('Phone') }} : {{ $pharmacy->phone }}</li>
                                                                <li>{{ \App\CPU\translate('City') }} : {{ $pharmacy->cities['name'] }}</li>
                                                                <li>{{ \App\CPU\translate('Address') }} : {{ $pharmacy->address }}</li>
                                                                <li>{{ \App\CPU\translate('Working for time') }} : {{ $pharmacy->working_from_time }}</li>
                                                                <li>{{ \App\CPU\translate('Working to time') }} : {{ $pharmacy->working_to_time }}</li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                </div>

                                                @php($reviews_of_pharmacy = App\Model\ReviewsOtherType::where('type','pharmacy')->where('type_id',$pharmacy->id)->paginate(2))
                                                <div class="tab-pane fade" id="reviews" role="tabpanel">
                                                            ...
                                                            <div class=" row d-flex justify-content-center align-items-center">
                                                                ...
                                                                <div class="col-12 d-flex justify-content-center align-items-center mt-2">
                                                                    <span class="text-center">
                                                                        {{$reviews_of_pharmacy->total()}} {{\App\CPU\translate('ratings')}}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div class="row pb-4 mb-3">
                                                        <div style="display: block;width:100%;text-align: center;background: #F3F4F5;border-radius: 5px;padding:5px;">
                                                            <span class="text-capitalize">{{\App\CPU\translate('Pharmacy Review')}}</span>
                                                        </div>
                                                    </div>
                                                    <div class="row pb-4">
                                                        <div class="col-12" id="pharmacy-review-list">
                                                            {{-- @foreach($reviews_of_doctor as $doctorReview)
                                                                @include('web-views.partials.doctor-reviews',['doctorReview'=>$doctorReview])
                                                            @endforeach --}}
                                                            @if(count($pharmacy->reviews)==0)
                                                                <div class="card">
                                                                    <div class="card-body">
                                                                        <h6 class="text-danger text-center">{{\App\CPU\translate('pharmacy_review_not_available')}}</h6>
                                                                    </div>
                                                                </div>
                                                            @endif
                                            ...
                                            <div class="p-4">
                                                <div class="container rtl" style="text-align: left;">
                                                    <div class="row no-gutters">
                                                        <div class="col-lg-12 for-send-message py-3 px-4 px-xl-5  box-shadow-sm">
                                                            <h2 class="h4 mb-4 text-center"
                                                                style="color: #030303; font-weight:600;">{{ \App\CPU\translate('Request Aprescription') }}</h2>
                                                                <form action="{{ route('pharmacy-order',[$pharmacy->id])}}" method="POST" id="getResponse" enctype="multipart/form-data">
                                                                    @csrf
                                                                <div class="row">
                                                                    <div class="col-sm-6">
                                                                        <div class="form-group">
                                                                            ...
                                                                            <input class="form-control name"
                                                                                name="name" type="text"
                                                                                value="" placeholder="{{ \App\CPU\translate('Mohamed Ahmed') }}"
                                                                                required="">
                                                                        </div>
                                                                    </div>
                                                                    <div class="col-sm-6">
                                                                        <div class="form-group">
                                                                            ...
                                                                            <input class="form-control email"
                                                                                ...
                                                                                value=""
                                                                                placeholder="{{ \App\CPU\translate('test@gmail.com') }}" >
                                                                        </div>
                                                                    </div>
                                                                    <div class="col-sm-6">
                                                                        <div class="form-group">
                                                                    ...
                                                                    <div class="col-sm-6">
                                                                        <div class="form-group">
                                                                            <label for="cf-subject">{{ \App\CPU\translate('Prescription Image') }}</label>
                                                                            <input class="form-control subject" type="file" name="prescription_image" value="" required="">
                                                                        </div>
                                                                    </div>
                                                                    <div class="col-md-12">
                                                                        <div class="form-group">
                                                                            <label for="cf-message">{{ \App\CPU\translate('Message') }}</label>
                                                                            <textarea class="form-control message" name="message" rows="6" required="">{{ \App\CPU\translate('Message') }}</textarea>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                {{-- <div id="recaptcha_element" style="width: 100%;"
                                                                    data-type="image">
                                                                    <div style="width: 304px; height: 78px;">
                                                                        <div><iframe title="reCAPTCHA"
                                                                                src="https://www.google.com/recaptcha/api2/anchor?ar=1&amp;k=6LfMARoeAAAAAAITvA-le6X9IElSWX6CncicwEfY&amp;co=aHR0cHM6Ly82dmFsbGV5LjZhbXRlY2guY29tOjQ0Mw..&amp;hl=en&amp;type=image&amp;v=5JGZgxkKwe0uOXDdUvSaNtk_&amp;size=normal&amp;cb=vnbo66qn76f"
                                                                                width="304" height="78"
                                                                                role="presentation" name="a-d2izhr6eyzzw"
                    ...
                    <div class="product-details-shipping-details">
                        ...
                        <div class="shipping-details-bottom-border">
                            <div style="padding: 25px;">
                                ...
                                <span>{{ \App\CPU\translate('100% Authentic Pharmacy') }}</span>
                            </div>
                        </div>
                    </div>
                    <div style="padding: 25px;">
                        <div class="row d-flex justify-content-center">
                            <span style="text-align: center;font-weight: 700; font-size: 16px;">
                                {{ \App\CPU\translate('More Pharmacies') }}
                            </span>
                        </div>
                    </div>
                    <div style="">
                        @foreach($pharmacies_city as $ph_city)
                            @php($overallRating = \App\CPU\ProductManager::get_overall_rating($ph_city->reviews))
                            <div class="flash_deal_product rtl" style="cursor: pointer; height:155px; margin-bottom:10px;"
                                onclick="location.href='{{route('pharmacyView',[$ph_city->id])}}'">
                                    ...
                                    <div class=" d-flex align-items-center justify-content-center"
                                        ...
                                        <div class="flash-deals-background-image" style="background: #3b71de10">
                                            <img style="height: 125px!important;width:125px!important;border-radius:5px;"
                                                src="{{asset("storage/app/public/pharmacies/thumbnail/$ph_city->thumbnail")}}"
                                                onerror="this.src="{{asset("storage/app/public/pharmacies/thumbnail/$ph_city->thumbnail")}} />
                                        </div>
                                    </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="flash-product-review">
                                                ...
                                                <label class="badge-style2">
                                                    ( {{$ph_city->reviews->count()}} )
                                                </label>
                                            </div>
                                            <div class="flash-product-price">
                                                {{ \App\CPU\translate('Working for time') }} : {{$ph_city->working_from_time}}
                                            </div>
                                            <div class="flash-product-price">
                                                {{ \App\CPU\translate('Working to time') }} : {{$ph_city->working_to_time}}
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <a class="btn-scroll-top" href="#top" data-scroll>
            <span class="btn-scroll-top-tooltip text-muted font-size-sm mr-2">{{ \App\CPU\translate('Top') }}</span>
...
@push('script')
    <script>
        ...
        function load_review()
        {
            ...
            $.ajax({
                    type: "post",
                    url: '{{route('review-list-pharmacy')}}',
                    data:{
                        type_id:{{$pharmacy->id}},
                        offset:load_review_count
                    },
                    success: function (data) {
                        $('#pharmacy-review-list').append(data.pharmacyReview)
