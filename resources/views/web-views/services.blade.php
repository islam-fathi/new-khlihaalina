...
@section('title',\App\CPU\translate('All Services Page'))
...
@section('content')
    ...
    <div class="container mb-md-4">
        <div class="row mt-3 mb-3 border-bottom">
            <div class="col-md-8">
                <h4 class="mt-2">{{ \App\CPU\translate('Services') }}</h4>
            </div>
            <div class="col-md-4">
                <form action="{{route('search-service')}}">
                    @csrf
                    <div class="input-group mb-3">
                        <input type="text" class="form-control"  placeholder="{{\App\CPU\translate('Service name')}}" name="service_name" required>
        ...
        <div class="row">
            ...
            <section class="col-lg-12">
                ...
                <div class="row mx-n2" style="min-height: 200px">
                    ...
                    @php($now = now()->setTimeZone($destinationTimezone)->format('H:i'))
                    @foreach($services as $service)
                    @if($now >= $service->working_from_time )
                    @else
                    @endif
                        <div class="col-lg-2 col-md-3 col-sm-4 col-6 px-2 pb-4 text-center">
                            <div class="card-body shadow">
                                <a href="{{route('serviceView',['id'=>$service->id])}}">
                                    ...
                                    <div class="text-center text-dark">
                                        <span class="text-center font-weight-bold small p-1">{{Str::limit($service->name, 14)}}</span>
                                    </div>
                                </a>
                            </div>
                        </div>
