...
@section('title',\App\CPU\translate('All Doctors Page'))
...
@section('content')
    ...
    <div class="container mb-md-4">
        <div class="row mt-3 mb-3 border-bottom">
            <div class="col-md-8">
                <h4 class="mt-2">{{ \App\CPU\translate('Doctors') }}</h4>
            </div>
            <div class="col-md-4">
                <form action="{{route('search-doctor')}}">
                    @csrf
                    <div class="input-group mb-3">
                        <input type="text" class="form-control"  placeholder="{{\App\CPU\translate('Doctor name')}}" name="doctor_name" required>
                    ...
                    @foreach($doctors as $doc)
                        <div class="col-lg-2 col-md-3 col-sm-4 col-6 px-2 pb-4 text-center">
                            <div class="card-body shadow">
                                <a href="{{route('doctorView',['id'=>$doc->id])}}">
                                    <img style="vertical-align: middle;height: 6rem; border-radius: 3%;"
                                        onerror="this.src='{{asset('public/assets/front-end/img/image-place-holder.png')}}'"
                                        src="{{asset("storage/app/public/doctors/thumbnail/$doc->thumbnail")}}"
                                        alt="{{$doc->name}}">
                                    <div class="text-center text-dark">
                                        <span class="text-center font-weight-bold small p-1">{{Str::limit($doc->name, 14)}}</span>
                                    </div>
                                </a>
                            </div>
                        </div>
