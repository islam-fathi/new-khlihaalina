    ...
    @include('web-views.partials._profile-aside')
        <section class="col-lg-9  col-md-9">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('review-other-type.store')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                @if($type == "doctor")
                                    ...
                                    <input name="type_id" value="{{$data_details->doctor_id}}" hidden>
                                    <input name="request_id" value="{{$data_details->id}}" hidden>
                                @elseif($type == "pharmacy")
                                    <input name="type" value="{{$type}}" hidden>
                                    <input name="type_id" value="{{$data_details->pharmacy_id}}" hidden>
                                    <input name="request_id" value="{{$data_details->id}}" hidden>
                                @elseif($type == "service")
                                    <input name="type" value="{{$type}}" hidden>
                                    <input name="type_id" value="{{$data_details->service_id}}" hidden>
                                    <input name="request_id" value="{{$data_details->id}}" hidden>
                                @endif()
