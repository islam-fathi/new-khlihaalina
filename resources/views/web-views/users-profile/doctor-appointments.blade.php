...
@section('title',\App\CPU\translate('My Doctor Appointments List'))
    ...
    <div class="container rtl" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
        <div class="row">
            ...
            <div class="col-md-9 mt-2 sidebar_heading">
                <h1 class="h3  mb-0 p-3 float-{{Session::get('direction') === "rtl" ? 'right' : 'left'}} headerTitle">{{\App\CPU\translate('my_doctor_appointments')}}</h1>
            </div>
        </div>
    </div>
                    ...
                    <div style="overflow: auto">
                        <table class="table">
                            ...
                            <tr style="background-color: #6b6b6b;">
                                <td class="tdBorder">
                                    <div class="py-2"><span
                                            class="d-block spandHeadO ">{{\App\CPU\translate('#')}}</span></div>
                                </td>
                                <td class="tdBorder">
                                    <div class="py-2"><span
                                            class="d-block spandHeadO"> {{\App\CPU\translate('Doctor Name')}}</span></div>
                                </td>
                                <td class="tdBorder orderDate">
                                    <div class="py-2"><span
                                            class="d-block spandHeadO">{{\App\CPU\translate('Appointment')}} {{\App\CPU\translate('Date')}}</span>
                                    </div>
                                </td>
                                <td class="tdBorder">
                                    <div class="py-2"><span
                            ...
                            <tbody>
                            @foreach($doctor_appointments as $order)
                                <tr>
                                    <td class="bodytr font-weight-bold">
                                        {{$order['id']}}
                                    </td>
                                    <td class="bodytr">
                                        @isset($order->doctors['name'])
                                            <span class="spanTr"> {{$order->doctors['name']}} </span>
                                        @endisset()
                                        @empty($order->doctors['name'])
                                            <span class="spanTr"> {{\App\CPU\translate('Doctor not found')}}</span>
                                        @endempty()
                                    </td>
                                    <td class="bodytr orderDate"><span class="">{{$order['created_at']}}</span></td>
                                    <td class="bodytr">
                                        @if( $order['appointment_status']=='canceled')
                                            <span class="badge badge-danger text-capitalize">
                                                {{\App\CPU\translate($order['appointment_status'])}}
                                            </span>
                                    ...
                                    <td class="bodytr">
                                        <a href="{{ route('doctor-appointment-details', ['id'=>$order->id]) }}"
